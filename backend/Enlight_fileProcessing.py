...
def apply_hamming_correction(signal: np.ndarray, time: np.ndarray) -> np.ndarray:
    """
...
def single_fft_analysis(signal: np.ndarray, time: np.ndarray, plot_title: str = "FFT Result"):
    ...
    fft_result = np.fft.fft(corrected_signal)
...
for k in range(0, numSensors):
    ...
    impacts_vibe = extract_impacts(dfVibe.iloc[:,k], timeSpanV)
    ...
    for i in range(impacts_vibe.shape[1]):
        row = i // 5
        col = i % 5
        axis[row, col].plot(impacts_vibe[:, i])
